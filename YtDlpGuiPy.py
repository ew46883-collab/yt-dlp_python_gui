...
class YtDlpGui:
    def __init__(self, root):
        ...
        self.video_resolution_setting = '-f "bv+ba"'
        ...
        self.translations = {
            ...
            "繁體中文": {"title": "YouTube 批量下載器專業版 "+self.version, "url_label": "影片連結", "param_label": "參數設置:",
                         ...
                         "menu_cut": "剪下", "menu_copy": "複製", "menu_paste": "貼上", "menu_select_all": "全選","resoultion" :"影片解析度",
                         "submit": "提交"
                         },
            ...
            "日本語": {"title": "YouTube一括ダウンロード Pro "+self.version, "url_label": "ビデオURL", "param_label": "引数",
                       ...
                       "menu_cut": "切り取り", "menu_copy": "コピー", "menu_paste": "貼り付け", "menu_select_all": "すべて選択","resoultion" :"ビデオ解析度",
                       "submit": "送信"
                       },
            "Français": {"title": "Téléchargeur Pro "+self.version, "url_label": "URLs", "param_label": "Arguments",
                         ...
                         "menu_cut": "Couper", "menu_copy": "Copier", "menu_paste": "Coller", "menu_select_all": "Tout sélectionner","resoultion" :"Résolution vidéo",
                         "submit": "Soumettre"
                         },
            "Italiano": {"title": "Downloader Pro "+self.version, "url_label": "URL", "param_label": "Argomenti",
                         ...
                         "menu_cut": "Taglia", "menu_copy": "Copia", "menu_paste": "Incolla", "menu_select_all": "Seleziona tutto","resoultion" :"Risoluzione video",
                         "submit": "Invia"
                         },
            "Español": {"title": "Descargador Pro "+self.version, "url_label": "URLs", "param_label": "Argumentos",
                        ...
                        "menu_cut": "Cortar", "menu_copy": "Copiar", "menu_paste": "Pegar", "menu_select_all": "Seleccionar todo","resoultion" :"Resolución video",
                        "submit": "Enviar"
                        },
            "Deutsch": {"title": "Downloader Pro "+self.version, "url_label": "URLs", "param_label": "Parameter",
                        ...
                        "menu_cut": "Ausschneiden", "menu_copy": "Kopieren", "menu_paste": "Einfügen", "menu_select_all": "Alles auswählen","resoultion" :"Video Auflösung",
                        "submit": "Einreichen"
                        },
            "Русский": {"title": "Загрузчик Pro "+self.version, "url_label": "URL", "param_label": "Аргументы",
                        ...
                        "menu_cut": "Вырезать", "menu_copy": "Копировать", "menu_paste": "Вставить", "menu_select_all": "Выбрать все","resoultion" :"Разрешение видео",
                        "submit": "Отправить"
                        },
            "한국어": {"title": "일괄 다운로더 Pro "+self.version, "url_label": "URL", "param_label": "매개변수", "sleep_label": "대기 시간 (초):",
                    ...
                    "menu_cut": "잘라내기", "menu_copy": "복사", "menu_paste": "붙여넣기", "menu_select_all": "전체 선택","resoultion" :"비디오 해상도",
                    "submit": "제출하다"
                    }
        }
        ...
        self.base_args =' --cookies "./cookies.txt" --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36 Edg/144.0.0.0" --js-runtimes node -o "%(title)s[%(id)s][%(upload_date)s].%(ext)s" --embed-thumbnail --convert-thumbnails png --merge-output-format mkv --ffmpeg-location "./ffmpeg"'
        self.DEFAULT_ARGS = self.video_resolution_setting + self.base_args
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        # 视频分辨率选择
        self.resoultion_lbl = tk.Label(nav_frame, font=("Arial", 10))  # 先创建
        self.resoultion_lbl.pack(side=tk.LEFT, padx=5)
        self.resoultion_textbox = tk.Entry(nav_frame, width=8)  # 建议给个宽度
        self.resoultion_textbox.pack(side=tk.LEFT, padx=2)
        self.resoultion_textbox.insert(tk.END, "720")
        self.result_submit_buttion = tk.Button(
            nav_frame,
            command=self.video_resolution_control,
            text=self.translations[self.current_lang]["submit"],
            height=1,
            width=10,
            bg="#2196F3",
            fg="white"
        )
        self.result_submit_buttion.pack(side=tk.LEFT, padx=5)
    ...
    def change_language(self, lang_name):
        ...
        self.resoultion_lbl.config(text=t['resoultion'])
        self.result_submit_buttion.config(text=t['submit'])
    ...
    def video_resolution_control(self):
        # 1. 从分辨率输入框获取当前用户输入的值
        res = self.resoultion_textbox.get().strip()

        # 2. 根据逻辑生成分辨率参数段 (逻辑遵循你的道十箴言：测量与比较)
        if res == '' or res == '0':
            new_res_setting = '-f "bv+ba"'
        else:
            new_res_setting = f'-f "bv*[height={res}]+ba"'

        # 3. 更新主参数框 (假设 self.DEFAULT_ARGS 是你原本那一长串参数)
        # 提取除了分辨率以外的其他参数部分，或者直接重新组合
        full_cmd=new_res_setting+self.base_args
        print(full_cmd)# 建议定义一个 BASE_TEMPLATE 存储固定参数

        self.param_input.delete(0, tk.END)
        self.param_input.insert(0, full_cmd)
        self.log(f">>> Resolution updated to: {res if res else 'Best'}")
